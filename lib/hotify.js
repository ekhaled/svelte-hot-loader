...
exports.reload = function (id, component) {
  ...
  componentMap[id].rollback = componentMap[id].component;
  ...
  return (function () {
    ...
    proxyComponent.prototype._register = function(options){
      ...
      try {
        ...
        var rb = componentMap[id].rollback;

        //resolve to previous working version of component
        if(rb){
          this.proxyTarget = new rb(options);
          console.info(this._debugName + ' rolled back to previous working version');
        }else{
          console.error("Could not rollback to previous working version of " + this._debugName)
        }
      }
    ...
    proxyComponent.prototype._rerender = function (oldComponent) {
